 ...
 * @param url
 */
async function markWeatherRequest(url) {
...
server.tool('lookUpCity', "获取城市的基本信息，包括城市的Location ID（你需要这个ID去查询天气），多语言名称、经纬度、时区、海拔、Rank值、归属上级行政区域、所在行政区域等", {
    location: z.string().describe("The location to look up"),
}, async ({ location }) => {
    const cityData = await markWeatherRequest('/geo/v2/city/lookup?location=' + location);
    return {
        content: [{
                type: "text",
